...
Replace the rehearsal syntax with `called_with`. See the [`when` guide][when-guide] for more details.
...
The `ignore_extra_args` option is still passed to `when`, not `called_with`.
...
Replace the rehearsal syntax with `called_with`. See the [`verify` guide][verify-guide] for more details.
...
The `times` and `ignore_extra_args` options are still passed to `verify`, not `called_with`.
...
Use [`When.get`][decoy.next.When.get], [`When.set`][decoy.next.When.set], and [`When.delete`][decoy.next.When.delete], to configure attribute access stubs.
