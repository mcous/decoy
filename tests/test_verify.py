...
def test_verify_attribute_set(decoy: Decoy) -> None:
    ...
    with decoy.verify_attributes() as verify:
        verify(subject.some_property).set_with("42")

        with pytest.raises(errors.VerifyError) as exc_info:
            verify(subject.some_property).set_with("43")
...
def test_verify_attribute_delete(decoy: Decoy) -> None:
    ...
    with decoy.verify_attributes() as verify:
        verify(subject.some_property).delete()

        with pytest.raises(errors.VerifyError) as exc_info:
            verify(subject.other_property).delete()
