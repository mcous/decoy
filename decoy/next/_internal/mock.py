...
from .errors import createNotAMockError
from .inspect import (
    bind_args,
    get_awaitable_value,
    get_child_spec,
...
class MockInternals:
    ...
    def call(self, args: tuple[object, ...], kwargs: dict[str, object]) -> object:
        ...
        behavior = self.state.use_behavior(
            mock=self.info,
            event=event,
            event_state=self.event_state,
        )

        if not behavior.is_success and behavior.expected_events:
            warnings.warn(
    ...
    def has_attribute_value(self, name: str) -> bool:
        return name in self.attribute_values and not self.state.is_verifying_attributes

    def get_attribute_value(self, name: str) -> object:
...
class Mock:
    ...
    def __getattr__(self, name: str) -> Any:
        ...
        if self._decoy.has_attribute_value(name):
            return self._decoy.get_attribute_value(name)
    ...
    def __setattr__(self, name: str, value: object) -> None:
        child = self._decoy.get_child(name)
        self._decoy.attribute_values[name] = value
    ...
    def __delattr__(self, name: str) -> None:
        child = self._decoy.get_child(name)
        self._decoy.attribute_values.pop(name, None)
        self._decoy.state.use_behavior(
            mock=child._decoy.info,
...
def ensure_mock(method_name: str, maybe_mock: object) -> MockInfo:
    ...
    raise createNotAMockError(method_name, maybe_mock)
