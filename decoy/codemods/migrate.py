...
from libcst.metadata import BaseAssignment, ParentNodeProvider, Scope, ScopeProvider
...
class MigrateCommand(VisitorBasedCodemodCommand):
    ...
    def migrate_import(
        ...
        keep_imports = [
            ...
            if IMPORT_MIGRATIONS.get(a.evaluated_name) == ImportAction.KEEP
        ]
        next_imports = [
            _migrate_alias(a)
            for a in import_aliases
            if IMPORT_MIGRATIONS.get(a.evaluated_name) == ImportAction.NEXT
        ]
                                ...
                                matcher=(
                                    ...
                                    | m.Arg(
                                        m.BooleanOperation(
                                            ...
                                            operator=m.Or(),
                                            right=m.List(),  # false branch (empty)
                                        ),
                                        star="*",
                                    )
                                ),
    ...
    def migrate_captor_arg(
        ...
        if name in scope and any(a in self._captor_assignments for a in scope[name]):
            return updated_node.with_changes(
                value=cst.Attribute(value=name_node, attr=cst.Name("arg"))
            )
...
def _migrate_alias(alias: cst.ImportAlias) -> cst.ImportAlias:
    rename = IMPORT_RENAMES.get(alias.evaluated_name)
    return alias.with_changes(
        name=cst.Name(rename) if rename else alias.name,
            ...
            cst.SimpleStatementLine(
                body=[
                    cst.Expr(
                        _migrate_rehearsal(
                            decoy_func,
                            _as_call(cst.ensure_type(element, cst.Element).value),
                            kwargs,
                        )
                    )
                ]
...
def _as_call(value: cst.BaseExpression) -> cst.Call:
    if m.matches(value, m.Await(m.Call())):
        value = cst.ensure_type(value, cst.Await).expression
    return cst.ensure_type(value, cst.Call)
